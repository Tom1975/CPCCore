...
/*
Tout le fonctionnement de la chaine t�te de lecture vers le PPI est document� : c'est le sch�ma du CPC :P

Pour faire simple (� partir des sch�ma dans l'amendment service manual) :
Le 464 :
1er �tage (transistor): pre-ampli du signal de la t�te de lecture (~ x37) + filtre passe haut du 1er ordre (FC=7 hz)
2eme �tage (AO) : 1er �tage d'amplification (~ x70) + filtre passe haut du 1er ordre (FC= 500Hz)
(d'apres tech manual : 0.330 -> 0.520 mV en sortie...)
3eme �tage (AO) : 2eme �tage d'amplification (~ x7)
Soit, d'apres manual : 2.31 -> 3.64 ??

Le 6128 poss�de uniquement un filtre passe haut du 1er ordre (FC=250Hz) et le 2eme �tage d'amplification (les deux autres sont dans le magn�to)

Dans les deux cas, la sortie des amplis est centr�e sur 1.7V et attaque directement l'entr�e du 8255, dont la tension de seuil haut est de 2.2V.

Les points les plus importants sont :
Le filtre passe haut a 250/500Hz et la sortie centr�e sur 1.7V.
- Le filtre passe haut permet le retour � 0 de l'entr�e du PPI lors des pauses.
- Le seuil d'entr�e du PPI � 2.2v (� comparer au 1.7V) qui permet d'ignorer les signaux trop faible.

Pour simuler �a, il suffit d'appliquer le filtre passe haut, un offset de 1.7V et un comparateur a 2.2V. Le seul probl�me est le gain, qui d�pend �videment de la source.
*/
...
*/
   ...
   // Pas ok sur la toute fin de "After the war", mais � investiguer car peu compr�hensible.
   fc_hp_ = 700;
   fc_lp_ = 6200;
   filter_gain_ = 0.015F;
   filter_order_hp_ = 1;
...
//////////////////////////////////////////////////
// High pass filter, Butterworth, fc et ordre param�trable.
//////////////////////////////////////////////////
// 01 : Bessel
// ...
...
int CTape::SampleToPPI ( double val )
{
   ...
   // Sortie centr�e sur 1.7v ->
   //val += 1.7;

   // Entr�e du PPI a 2.2v, on coupe donc les signaux trop faibles
...
void TriggerSchmitt ( double* value_tab, int nb_samples, double vtp, double vtm, double vsat )
{
   ...
   // Comparateur � deux seuils inverseur (de schmitt)
   for (int i = 0; i < nb_samples; i++)
   {
      // Depending on state
